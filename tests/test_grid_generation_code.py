...
def main ():
    ...
    study_area = ee.FeatureCollection("WWF/HydroSHEDS/v1/Basins/hybas_3") \
        .filterBounds(ee.Geometry.Point([-62.28, -6.35]))
    
    # Define the export collection
    export_collection = 'Amazon_Forecasting'
...
def generate_study_area_fishnet (study_area, username, export_collection):
    ...
    sample_grid = generate_fishnet(study_area, 40000).map(assign_grid_id)
    
    # Export the image to an image collection
    task = ee.batch.Export.table.toAsset(
        collection = sample_grid, 
        description = "Export-Grid-Strata", 
        assetId = "users/" + username + "/" + export_collection + "/export_grid_40km"
        )
    task.start()
