...
if __name__ == "__main__":
    ...
    tile_dir = "/media/john/Expansion/coca_inference"
    ...
    for file_path in file_paths:
        ...
        for current_index in range(0, (end_year-start_year) + 1):
            ...
            # if current_index in [0, 5, 10, 15, 20, 25, 30, 35]:
            ...
            pool_inputs.append(com2)
        
    # Run the pool processing
    starttime = time.time()
    pool = Pool(10)
    pool.map(run_sys_command, pool_inputs)
    pool.close()
    endtime = time.time()
